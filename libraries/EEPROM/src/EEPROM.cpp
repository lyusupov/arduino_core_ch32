/*
  ...
  The remainder can be used for EEPROM values and their inverse. The method to write these bytes can be different per chip. 
	 ...
	 he he - formula is "(EEPROM_SIZE/2)-6" as 6 bytes are used for config and each byte is stored 2x
...
// is not the same OB_USER_BFB2 defined in ch32v20x_flash.c ???
...
void EEPROMClass::begin(void)
{
  ...
  _size = 58;   // Option bytes available on CH32V002/V006, CH32X035/X033: 256- 16 = 240; 240/2=120, 2+120=122
#else  
  ...
  if(!_data)
  {
    ...
    for(size_t i=2; i<_size; i++)
      _data[i]=(uint8_t)ob16p[(OB_AVAIL_DATA_START-2)+i]; // better optimalization, constants are pre calculated
  }
  _dirty = false;
}
...
{ // Write the _data array to the available area for option bytes
  ...
  FLASH->KEYR = FLASH_KEY1;		// checking ch32v20x_flash.c line 502 -> seems no necessary
